...
(*-------------------------------------------------------------------------*)
  ...
  var i: integer;
  begin
    for i := 1 .. maxids do s[i] := lcase(s[i]);
  end;
  ...
  begin
    m := true;
    for i := 1 .. reslen do if lcase(a[i]) <> lcase(b[i]) then m := false;
    for i := reslen+1 .. maxids do if b[i] <> ' ' then m := false;
    strequri := m
  end { equstr };
      ...
      procedure statement(fsys: setofsys);
        ...
        begin llc := lc;
          ...
          if sy = ident then
            ...
            begin error(2); skip(fsys + [becomes,tosy,downtosy,dosy,range]) end;
          if sy = becomes then
            begin insymbol; expression(fsys + [tosy,downtosy,dosy,range]);
              if gattr.typtr <> nil then
            ...
            begin error(51); skip(fsys + [tosy,downtosy,dosy,range]) end;
          if sy in [tosy,downtosy,range] then
              ...
              if gattr.typtr^.form <> scalar then error(144)
                  ...
                  if comptypes(lattr.typtr,gattr.typtr) then
                    begin
                      ...
                      if (lsy = tosy) or (lsy = range) then gen2(52(*leq*),ord(typind),1)
          ...
          gattr := lattr; load;
          if (lsy = tosy) or (lsy = range) then gen1t(34(*inc*),1,gattr.typtr)
